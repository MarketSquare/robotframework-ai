...
class PhoneNumberGenerator(TestDataGenerator):
    ...
    def create_prompt_message(self, amount:int, format:str, phone_number_kwargs:dict):
        ...
        if format is None:
            system_message += f", phone number format is not defined fully, but use this prefix: {prefix}"
        else:
        ...
        if country is not None and mix_format is False:
            ...
            user_message = f"Give me a list of {amount} different real phone numbers from {country} in different but valid formats."
        else: # Country is None and mix_format is True
            user_message = f"Give me a list of {amount} different real phone numbers from different countries in the world in different but valid formats."
